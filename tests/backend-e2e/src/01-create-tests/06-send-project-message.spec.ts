...
test('should send message to project chat', async ({ projectId }) => {
  const messageContent = 'Test message from E2E test';
  ...
  const subscribeMessage = `SUBSCRIBE\nid:sub-0\ndestination:/join/projects/${projectId}\n\n\x00`;
  ...
  const sendMessage = `SEND\ndestination:/send/projects/${projectId}\ncontent-type:application/json\n\n${JSON.stringify({
    token: token,
