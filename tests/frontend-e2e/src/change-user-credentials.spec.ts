...
test('check reset button in change user credentials form', async ({ page }) => {
  ...
  await page.getByRole('textbox', { name: 'Enter first name' }).fill('Izka');
  ...
  await page.getByRole('textbox', { name: 'Enter new email' }).fill('isabella.garcia@example.com');

  // sprawdzenie wartości w polach formularza do edycji danych użytkownika przed zresetowaniem
  await expect(page.getByRole('textbox', { name: 'Enter first name' })).toHaveValue('Izka');
  await expect(page.getByRole('textbox', { name: 'Enter last name' })).toHaveValue('Gracias');
  await expect(page.getByRole('textbox', { name: 'Enter new email' })).toHaveValue('isabella.garcia@example.com');
